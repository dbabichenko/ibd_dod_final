...
def clean_longitudinal_inputs(load_pkl=True, padded=True, exclude_ct=True):
    ...
    utils.pkl_dump(x_mean, 'pickles/x_mean.pkl')
...
def fit_scaler(df):
    ...
    # df.to_pickle('pickles/x_padded_unscaled.pkl')
    # print("Saved unnormalized input data to pickles/x_padded_unscaled.pkl")
...
class Imputer:
    def __init__(self, df=None):
       ...
       'ENC_TEL_Unrelated', 'ENC_SURGERY', 'ENC_PK_INJ', 'DIAG_COLONOSCOPY',
...
class IBDSequenceGenerator:
    ...
    def write_to_disk(self, datadir=None, tgt_types=None, labels_only=False, seq_only=False):
        ...
        utils.pkl_dump(x_mean, os.path.join(out_fp, 'x_mean.pkl'))
...
def get_ttsplit_datasets(datadir, dataset_name, train_size=0.8, stratify_by=None, holdout_patients=[]):
    """
...
def get_patsplit_datasets(datadir, dataset_name, train_size=0.8, stratify_by=None):
    """
    Populate Tr/V/Te Datasets with {datadir}/{sequences}/*.npy and {datadir}/*_label_dict.pkl
