...
train_iter = DataLoader(train_ds, shuffle=True, batch_size=int(args.batch_size), drop_last=True, num_workers=5)
valid_iter = DataLoader(utils.pkl_load(os.path.join('datasets', args.dataset, 'valid.dataset')), shuffle=True, batch_size=int(args.batch_size), drop_last=True, num_workers=5)
...
tr_lb = meta_ds['train_labels']
nb_tgt_classes = len(tr_lb)
# class_weights = [sum(tr_lb)/(nb_tgt_classes * c) for c in tr_lb]
class_weights = [max(tr_lb)/c for c in tr_lb]

print(f"Loaded DataLoaders. Meta:{meta_ds}\nClass Weights: {class_weights}\n")
t0 = time.time()
model = gru.train_model(train_iter, valid_iter, X_Mean, args.OP_tgt, aux_cols, int(args.epochs), args.model_name, int(nb_tgt_classes), float(args.learning_rate), float(args.aux_alpha), float(args.tr_alpha), class_weights=class_weights)
