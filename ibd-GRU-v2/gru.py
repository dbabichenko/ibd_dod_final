...
def train_epoch(model, train_iter, tgt_col, aux_cols, criterion, optimizer, aux_alpha, tr_alpha,\
    ...
    for batch, (X, y_dict) in enumerate(train_iter):
        ...
        # TODO Unstable behavior. Without this, no '1' labelled samples were being seen. Check later.
        # if y_dict[tgt_col].unique(return_counts=True)[1][1]>0:
        #     print(f".", end='')
...
def predict(model, X,y, h1, h2):
    device = utils.try_gpu()
    yhat, _  = model.predict(X.to(device).float(), h1, h2) # We don't need cell state
...
def eval_model(model, test_iter, tgt_col, nb_classes):
    ...
    with torch.no_grad(): # require_grad = False
        ...
        eval_scores.update(classification_report(preds))
        model.train()
        return eval_scores
